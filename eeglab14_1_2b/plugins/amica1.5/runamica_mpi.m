...
function [weights,sphere,mods] = runamica13(dat,varargin)
...
if 1
MPI_BIN = '/home/jason/mpich2-1.5-install/bin/';
...
pcakeep = -1;
   ...
   for i = 1:2:length(varargin) % for each Keyword
      ...
      elseif strcmp(Keyword,'load_a')
         if isstr(Value)
            fprintf('runamica(): load_A should be 0 or 1');
            return
         else
            load_A = Value;
         end         
      elseif strcmp(Keyword,'load_sphere')
         if isstr(Value)
            fprintf('runamica(): load_sphere should be 0 or 1');
            return
         else
            load_sphere = Value;
         end         
      elseif strcmp(Keyword,'load_W')
         if isstr(Value)
            fprintf('runamica(): load_W should be 0 or 1');
            return
         else
            load_W = Value;
         end         
      ...
      elseif strcmp(Keyword,'use_node')
         if ~isstr(Value)
            fprintf('runamica13(): use_node argument must be a node name (string)');
            return
         else
            use_node = Value;
         end
         
      elseif strcmp(Keyword,'in_bg')
         if isstr(Value)
            fprintf('runamica13(): in_bg argument must be 0 or 1');
            return
         else
            in_bg = Value;
         end
      ...
      elseif strcmp(Keyword,'use_grad_norm')
         if isstr(Value)
            fprintf('runamica(): use_grad_norm must be 0 or 1');
            return
         else
            use_grad_norm = Value;
         end
      elseif strcmp(Keyword,'min_grad_norm')
         if isstr(Value)
            fprintf('runamica(): min_grad_norm must be a number');
            return
         else
            min_grad_norm = Value;
         end
      elseif strcmp(Keyword,'use_min_dll')
         if isstr(Value)
            fprintf('runamica(): use_min_dll must be 0 or 1');
            return
         else
            use_min_dll = Value;
         end
      elseif strcmp(Keyword,'min_dll')
         if isstr(Value)
            fprintf('runamica(): min_dll must be a number');
            return
         else
            min_dll = Value;
         end
...
elseif isstruct(dat)
    ...
    if strcmp(dat(end-3:end),'.set')
        ...
        file = [EEG.filepath filesep EEG.datfile];ls
...
max_threads = max(max_threads,numprocs);
...
if 0 % RUN_LOCALLY == 1
    ...
    if ~isempty(use_node)
        if in_bg
            system(['ssh ' use_node ' ' AMBIN ' ' outdir 'input.param &']);
        else
            system(['ssh ' use_node ' ' AMBIN ' ' outdir 'input.param']);
        end
    else
