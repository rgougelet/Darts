% runamica() - Perform AMICA -- adaptive ICA using multiple models with shared components
...
function modres = runamica10(file,outdir,chans,frames,varargin)
...
%
   for i = 1:2:length(varargin) % for each Keyword
      ...
      elseif strcmp(Keyword,'use_pool')
         if isstr(Value)
            fprintf('runamica(): use_pool must be 0 or 1');
            return
         else
            use_pool = Value;
         end
      ...
      elseif strcmp(Keyword,'scheduler')
         if ~isstr(Value)
            fprintf('runamica(): scheduler must be a string');
            return;
         else
            scheduler = Value;
         end  
...
if strcmpi(qsub, 'on')
    ...
    fprintf(fid,'#$ -cwd\n');
    if use_pool
        fprintf(fid,'#$ -q pool.q\n');
    end
    fprintf(fid,'#$ -j y\n');
    fprintf(fid,'#$ -S /bin/bash\n');
    fprintf(fid,'#$ -pe mpich %d\n',numprocs);
    
    % by default assuming that mpich-install is a sub-directory of the amica dir
    mpich_dir = [basepath 'mpich-install'];
    if ~exist(mpich_dir,'dir')
        mpich_dir = '/home/jason/mpich-install'; 
    end
    fprintf(fid,['MPI_DIR=' mpich_dir '\n']);
    
    fprintf(fid,...
            ['$MPI_DIR/bin/mpirun -np $NSLOTS -machinefile $TMP/machines ' basepath 'amica10 ' ...
             '%s' 'input.param'],outdir);
    fclose(fid);
    
    system(['ssh ' scheduler ' -n qsub ' outdir 'qsub.sh > ' outdir 'lastqsubid']);
    ...
    disp('Disregard any Warnings about X11 forwarding.');    
    disp(['qsub id = ' int2str(qsubid)]);
    

    disp('You can watch the last 25 lines of the output by typing:');
    disp(['   watch tail -n 25 qsub.sh.o' int2str(qsubid)]);
    disp(['in a terminal logged into ',scheduler]);
    disp('If it never starts, you may have to delete this job by typing:');
    disp(['   qdel ' int2str(qsubid)]);
    disp(['in a terminal logged into ' scheduler ', and then rerun.']);
    disp(['The processors used are in the file: qsub.sh.po' int2str(qsubid)]);
    disp(['When the qsub.sh.o' int2str(qsubid) ' file says, ... done, you can']);
    disp(['load the output at the matlab command line by typing:']);
    disp(['   >> mod = loadmodout10('' outdir '')']);
    disp(['or using the EEGLAB gui.']);

else
    system([basepath 'amica10 ' fullfile(outdir,'input.param') ]);
