...
block_size = 512; % initial block size
...
pcakeep = -1;
   ...
   for i = 1:2:length(varargin) % for each Keyword
      ...
      elseif strcmp(Keyword,'min_dll')
         if isstr(Value)
            fprintf('runamica(): min_dll should be lrate change to stop at');
            return
         else
            min_dll = Value;
         end
...
if RUN_LOCALLY == 1
    ...
    if 0
        ...
        if strcmp(use_pe,'mpich')
            % use mpich
            %fprintf(fid,...
            %    [MPI_BIN 'mpirun -np ' int2str(numprocs) ' -machinefile ' basepath 'machines ' AMBIN ' ' ...
            %    outdir 'input.param']);
            fprintf(fid,...
            [MPI_BIN 'mpirun -np ' int2str(numprocs) ' -machinefile $TMP/machines ' AMBIN ' ' ...
              outdir 'input.param']);
        else
            % use ORTE
            fprintf(fid,...
        ...
        system(['ssh `qconf -ss` -n ' MPI_BIN 'mpirun -np ' int2str(numprocs) ' -machinefile ' basepath 'machines2 ' ...
    ...
    %system(['ssh computing -n ' MPI_BIN 'mpirun -np ' int2str(numprocs) ' -machinefile ' basepath 'machines ' ...
    %            AMBIN ' ' outdir 'input.param']);
