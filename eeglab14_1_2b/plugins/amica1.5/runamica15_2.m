...
%machinefile = [basepath 'machines'];
machinefile = [basepath 'machines2'];
...
pcakeep = -1;
   ...
   for i = 1:2:length(varargin) % for each Keyword
      ...
      elseif strcmp(Keyword,'machinefile')
         if ~isstr(Value)
            fprintf('runamica(): machinesfile must be a string');
            return
         else
            machinesfile = Value;
            RUN_LOCALLY = 0;
...
if RUN_LOCALLY == 1
    ...
    system(['ssh `qconf -ss` -n ' MPI_BIN 'mpirun -np ' int2str(numprocs) ' -machinefile ' machinefile ' ' ...
