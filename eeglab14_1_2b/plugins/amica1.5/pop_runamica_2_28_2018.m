...
mpidlg = [mpidlg 'mpires = inputgui(''geometry'',{ '];
for k = 1:length(lds)
    mpidlg = [mpidlg '[1 3],'];
end
mpidlg = [mpidlg ' [1 3], [1 1 2]},''uilist'',{'];

%mpidlg = [mpidlg '{''style'' ''checkbox'' ''string'' ''Use MPI -- Location:'' ''value'' 0},'];
%mpidlg = [mpidlg '{''style'' ''text'' ''string'' ''$MPI_BIN''},'];

for k = 1:length(mchs)
    mpidlg = [mpidlg ['{''style'' ''checkbox'' ''string'' ''' mchs{k} ''' ''value'' 1},']];
    mpidlg = [mpidlg ['{''style'' ''text'' ''string'' ''' num2str(lds(k)) '''},']];
end
...
%mpidlg = [mpidlg 'ld = getloads(mchs); [mk,ord]=sort(ld); mchs{ord(1:numprocs)}'];
