...
function mods = runamica16(dat,varargin)

%%%%%%%%%%%%%%%%%%%%%%% Set up keyword default values %%%%%%%%%%%%%%%%%%%%%%%%%

binfile = 'amica15c'; % on computing

bytesize = 4; % not used, now automatically determined

AMBIN = '/home/jason/amica_open64/amica15c';
MPIBIN = '/home/jason/mpich-3.1.3-install/bin/';
NODELISTFILE = '/home/jason/nodelist2';
...
elseif isstruct(dat)
    ...
    if length(dat.filepath) == 0
        file = [pwd filesep dat.datfile];
    else
...
if 1 %RUN_LOCALLY == 1
    system(['ssh computing -n ' MPIBIN 'mpirun -np ' int2str(numprocs) ' --hostfile ' NODELISTFILE ' ' AMBIN ' ' outdir 'input.param']);
    mods = loadmodout12(outdir);
    disp('to import: EEG = eeg_loadamica( EEG, outdir, modnum )');
    weights = mods.W(:,:,1);
