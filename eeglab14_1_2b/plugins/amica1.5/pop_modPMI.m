function [EEG com] = pop_modPMI(EEG,varargin)
...
try
    ...
    disp('pop_modPMI() error: calling convention {''key'', value, ... } error'); return;
end;
try g.maxpass;               catch, g.maxpass       = 3;   end; 
...
for i = models2plot
    ...
    if ~(size(tmpData,2) == 0 || size(tmpData,3) == 0)
        ...
        if order
            ...
            [mo{i}, ord{i}] = arrminf2(MI,g.maxpass);
