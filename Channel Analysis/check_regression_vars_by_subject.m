...
parfor case_i = 1:length(cases)
	...
	if is_FIR
		...
		if reject_labeled_ics
			in_file = [in_file,' Reject ICs -'];
			if use_laplacian
				in_file = [in_file,' Laplacian'];
			else
				in_file = [in_file,' Linked Mastoid'];
			end
		else
			in_file = [in_file,' Keep ICs -'];
			if use_laplacian
				in_file = [in_file,' Laplacian'];
			else
				in_file = [in_file,' Linked Mastoid'];
...
% 	
	...
	for var = {'delay_Front_theta','delay_Front_alpha','delay_Front_gamma',...
		...
		h =	tight_subplot(1,2,.045,.1);
		t = annotation('textbox',[.5,.98,0,0],...
			'string',[strrep(uplow(var{:}),'_',' '),' by Subject x Condition'],...
			'FitBoxToText','on',...
			'LineStyle','none',...
			'HorizontalAlignment','center',...
			'FontSize',14);
		...
		tic; print(fig,[data_dir,' ',in_file,t.String],'-dsvg','-r300'); toc;
	end
end
